...
Current feature set: **Knowledge Continuity 1 + Rust Survival RS1–RS4 + Production Planning RP1 + Mentorship KF1 + WorldSim WM4.5 + Visible Autonomous Settlement Gameplay**, on the `0.5.0` engine/UI/base-save family. This file describes code and limits. The exact candidate verification run and the exact main Pages deployment are the release authority; neither a green older run nor this document proves a newer commit.
...
WM4.5 is active on production main. `src/worldsim-resource-authority.mjs` remains the single regeneration writer; food uses the conservative ecology-sensitive threshold policy selected after WM4.4 evidence. WM4.4 remains calibration/reference evidence, not the active writer.
...
| Ecology calibration / alternative formula evaluation | WM4.2–WM4.4 remain evidence/Formula Lab; WM4.5 conservative food policy is active |
