...
Menu interiors are also part of the V0.9 candidate: Main Menu, People, Survival, Rust, Systems and AI Decision Feed use visual hierarchy before prose. Survival now exposes six settlement metrics first, keeps Knowledge controls immediately available, and collapses Kingdom/WorldSim/Lifecycle detail; Rust uses visual recipes, slots and progress; People uses portrait cards and mini Need bars. Existing engine-command paths remain unchanged.
