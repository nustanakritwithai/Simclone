...
The world is also becoming the primary information surface. A selected/recent decision or real Need appears as a thought cloud; direct Knowledge/Mentor communication may appear as a speech bubble only when sender/receiver evidence exists; BUILD/CRAFT/PROCESS appears as a work bubble; incomplete housing keeps a spatial progress marker. Evidence-backed sender→receiver links may appear briefly, selected/recent work can show its authoritative task destination, and dropped Rust items are visible from the existing item ledger. Bubble selection is deterministic and presentation-only, capped at 3 simultaneous bubbles on mobile and 5 on larger viewports. No free-form dialogue, motive, thought history, item ledger or second event ledger is created. Long provenance/explanation remains available on demand. This candidate requires exact browser/CI evidence before release.
