...
UX V0.8 Event → Cause is released at `main@ccc058afa6b7f71b875895b4d4cf82e3d0837bda`. Exact candidate `5f1df461351a57a6dac57f3ec0d89b118c4b3c4b` passed Verify #566 and exact main Pages #58 succeeded. Chronicle event detail is evidence-first and read-only: it resolves historical cause only when the event tick can be matched to retained authoritative evidence such as lineage, immutable death history, career/skill provenance, Rust placement/item records, knowledge evidence, mentorship links or Cultural Archive records. Missing historical cause remains explicit `UNKNOWN`; the UI never substitutes a Clone's current decision trace for historical intent. No replay authority or new event-history ledger is introduced.

## UX V0.9 Visual-First + Diegetic release
Menu interiors are also part of the V0.9 candidate: Main Menu, People, Survival, Items/Rust, Systems and AI Decision Feed use visual hierarchy before prose. Building-specific interaction is no longer packed into Survival/Rust submenus. Camp, Crafting Table, Furnace and Modular House open contextual menus directly from their world-space objects. Camp owns Cultural Archive controls, Crafting Table exposes table-only recipes, Furnace exposes Charcoal processing, and House status reads the canonical housing evaluator. Survival remains an overview, while Items/Rust keeps possessions, hand recipes and global production policy. Existing engine-command paths remain unchanged.

Visual Analytics is now included in the V0.9 candidate. Camp renders an interactive Knowledge Graph from retained archive authors/entries and the selected Clone's actual beliefs; Chronicle graphs retained event counts across the latest seven simulation days; Survival and Systems use current-state bar charts; incomplete House context visualizes canonical missing-piece composition. Graph filters and selections are presentation-only, and no historical resource series or social/knowledge edge is invented.
...
The world is also becoming the primary information surface. A selected/recent decision or factual Need appears as a thought cloud; direct Knowledge/Mentor communication may appear as a speech bubble only when sender/receiver evidence exists; BUILD/CRAFT/PROCESS appears as a work bubble; incomplete housing keeps a spatial progress marker. Evidence-backed sender→receiver lines, selected living Parent/Child or active Mentor lines, task-target/resource pulses, dropped Rust item cues, recent Birth/Death bursts and Skill/Craft/Build achievement bursts are all derived from existing state/events. Bubble selection is deterministic and presentation-only, capped at 3 simultaneous bubbles on mobile and 5 on larger viewports. There is no general Emotion Authority, so low satiety/energy/HP may be shown as factual Needs but are never relabeled as sadness, fear, anger or happiness. No free-form dialogue, motive, thought history, item ledger or second event ledger is created. Long provenance/explanation remains available on demand. This candidate requires exact browser/CI evidence before release.

## UX V1.0 World Interaction Complete candidate

V0.9 was merged at `main@df93ba3c0aff768f7c857eaefba271a6a7ac3f12` after exact Verify #724 passed. Public Pages #59 was queued at the last release check and remains a separate public-proof gate.

V1.0 starts from that merge and extends direct world interaction without changing simulation authority. Resource nodes now open their own amount/max/current-worker context, dropped Rust items open item-instance/provenance context with pickup routed through `PICKUP_ITEM`, and recent life/achievement event bursts can open the existing evidence-first event detail directly. Event-marker hit priority is limited to the visible marker so normal Clone selection remains intact. No resource, item or event ledger is added.
