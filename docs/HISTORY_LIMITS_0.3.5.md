...
- Living gameplay capacity remains min(housing, 36). Retained capacity is 1024 total identities; archive JSON budget is 1800000 JavaScript string code units. Whole-save serialize/restore budget is 2500000 code units. The previous 2000000-character budget became too small under the verified autonomous-housing demographic path, so this hotfix raises only the bounded whole-save guard; retained-count and archive limits remain unchanged. These are not UTF-8 bytes or a promise about browser quota.
