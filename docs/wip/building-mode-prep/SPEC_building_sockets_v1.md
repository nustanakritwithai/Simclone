...
## การแก้หลังรีวิว PR #67 (25 ก.ย. 2026)
1. ไม่บังคับ bump `SAVE_VERSION`; ให้ `RS3-0.2 → RS3-0.3` เป็น marker ของการเปลี่ยน save และปิด Open question 9
2. คง RP1 ไว้ที่ version เดิม ไม่เก็บ `productionPlan.house`; derive แผนบ้านจาก world state ทุกครั้ง และปรับชื่อ goal ใน history
3. ย้าย `snapSocket`/การ snapping ไป phase (c) Structural Snap; phase (a) ตรวจเฉพาะ canonical socket และ `socket-shape`
4. ย้ายกรณี refund ไป P3 ระบุลำดับ duplicate ก่อน actor และระบุจังหวะหักไม้กับการใช้ item
5. ปรับ RP1-off test ให้เทียบ hash ของ nodes, agents, stock และ `ecologySignature` กับ baseline โดยยอมให้ full serialize ต่างจาก RS3/`placements`
6. ระบุว่า grep `*6` ใน §5.2 ตรวจเฉพาะ capacity formula ไม่รวม `kingdom-market.mjs` ที่เป็นราคา
...
- ปุ่ม `place-station` (`ux.mjs` บรรทัด 146–150) ที่เลือกช่องข้างตัวเองแบบอัตโนมัติ: สำหรับ structure piece ให้เปลี่ยนเป็น Building Mode ใน phase (b) และทำ snapping ใน phase (c) Structural Snap (ข้อ 3) ส่วน table/furnace ใช้แบบเดิมต่อได้

---

## 3. Structural Snap (phase (c)) — snap ก่อน แล้วค่อยคำนวณ support/level

ข้อกำหนด `snapSocket`/การ snapping นี้ทำใน phase (c) Structural Snap ตาม §10; phase (a) ไม่ต้องมี snap UI แต่ executor ต้องปฏิเสธ socket ดิบที่ไม่ canonical ตามข้อ 1.2
...
- ทุก `PLACE_STATION` ของ structure piece ต้องมี `placementId` เป็น string; `placementIdFor` เป็น convention ของ caller ที่ RP1/UI **ต้องใช้** และควรสร้างรูปแบบ deterministic `pl:<tick>:<agentId>:<itemInstanceId>`
  - executor **ไม่ validate รูปแบบ string** และรับ string id ใดๆ เป็น key; UI ใช้ `state.tick` ขณะ confirm, engine BUILD task ใช้ `s.tick` (ข้อ 6.3)
...
- refund ในอนาคต (ถอดหรือพังแล้วคืนของ) อยู่ใน P3: **ต้องสร้าง item id ใหม่** (`nextItem++`) ห้ามนำ id เก่ากลับมาใช้ command เก่าที่ retry; replay ที่มี `placementId` เดิมต้องคืน `duplicate:true` หรือ `duplicate-item` ก่อนตรวจ actor แล้วจึงค่อยกำหนด expectation ของ P3 ใหม่ **วันนี้ยังไม่มี command ถอดหรือ refund ในโค้ด** (ค้น `REMOVE|DEMOLISH|destroy|salvage` ใน `src/` ไม่เจอ)
...
เพิ่ม test ที่ grep ว่าไม่มี `*6` หรือ `.length*6` ที่ใช้ใน **capacity formula** หลงเหลือใน `src/` นอก `housing.mjs` (เช่น `complete).length*6`); `kingdom-market.mjs` ที่เป็น `base*6` คือสูตรราคา ไม่เกี่ยวกัน
...
- ชื่อ goal ใน history อาจเป็น `house-site`, `craft-WOOD_FOUNDATION|WALL|DOORWAY|ROOF`, `equip-HAMMER-house`, `place-house-piece`, `house-complete` (validator รับ string อะไรก็ได้ ตาม `production-planning.mjs` บรรทัด 143)
...
1) ถ้า needsHouse(s) ให้ derive `house`/site จาก world state ทุกครั้ง (single source of truth) โดยใช้ `selectHouseSite(s,isWalkable)` เมื่อยังไม่มี component ที่กำลังสร้าง
      ...
      doorSide = ด้านที่ midpoint ใกล้ camp ที่สุด (เสมอกันให้ N<E<S<W); **ไม่อ่านหรือเขียน `productionPlan.house`**
...
4) ถ้ากระเป๋าของ builder ยังไม่มี piece.kind และ stock.wood >= recipe.wood + BIRTH_RULES.woodSafetyFloor(12)
      → rustCommand('CRAFT_ITEM',{agentId:builder.id, recipeId:piece.kind})   (หักไม้ครั้งเดียวเมื่อ craft order ถูก accept)
5) การวางจริงไม่ใช่งานของ RP1 แต่เป็นงานของ BUILD task ใน scheduler (ข้อ 6.3) และการวางใช้/ลบเฉพาะ item ที่ถืออยู่ ไม่หักไม้อีก
6) ถ้าบ้านเสร็จ (evaluate แล้ว complete) → record 'house-complete'
7) ถ้า site ใช้ไม่ได้แล้ว (มีของมาวางทับ foundation socket ก่อน) → derive site ใหม่ในรอบถัดไป
```
- `productionPlan` คง version **`RP1-0.2`**; ไม่เพิ่ม/ไม่เก็บ `productionPlan.house` และไม่ทำ migration ของ RP1 เพราะแผนบ้าน/ตำแหน่ง derive จาก world state ทุกครั้ง (single source of truth); `validateProductionPlan` จึงไม่ต้องตรวจ field นี้
...
- `engine.mjs` บรรทัด 199–203: ลูปเดิมที่วน `buildings.filter(!complete)` ให้เปลี่ยนเป็น `for (const p of pendingPlacements(s,a))` จาก `housing.mjs` โดยอ่าน derived house plan จาก world state + `nextHousePiece` + กระเป๋าของ `a` แล้วเพิ่ม candidate `'BUILD'` ที่ anchor cell ของ socket (base 56 เท่าเดิม) มีเฉพาะคนที่ถือชิ้นนั้นเท่านั้น
- `execute` บรรทัด 251–255: เมื่อถึงที่ ให้เรียก `command(s,'PLACE_STATION',{agentId,itemInstanceId,socket,placementId:placementIdFor({tick:s.tick,agentId:a.id,itemInstanceId:itemId})})` **ผ่าน command() → rustCommand → placeStationFromItem** (executor เดียว) และถ้า `reason==='hammer'` ให้เรียก `command('EQUIP_ITEM')` ก่อนหนึ่งครั้ง (authority เดิม ไม่เขียน equipment เอง)
...
- UI phase (a): ลบ panel "บ้านพักใหม่" (`ux.mjs` บรรทัด 94–108), tap-to-BUILD (`app.mjs` บรรทัด 247) และ shelter ghost (`ux.mjs` บรรทัด 163–170) **โดยไม่มี UI ทดแทน**; RP1 builds houses automatically ส่วน Building Mode/ghost อยู่ใน phase (b) และ snapping อยู่ใน phase (c) ตาม §10
...
- `productionPlan`: คง `RP1-0.2` และไม่เพิ่ม `house`; แผนบ้าน derive จาก world state ทุกครั้ง และ goal `'build-shelter'` ใน history เดิมเก็บไว้ได้ (เป็น string)
...
- `SAVE_VERSION`: **ไม่ต้อง bump top-level** และไม่ต้องเพิ่มกิ่ง `0.5.0→0.6.0` จากการเปลี่ยนนี้; marker ของ save change คือ migration `RS3-0.2 → RS3-0.3` ตามข้อ 7.2, storage key `simclone:world:v1` คงเดิม
...
4. `SAVE_VERSION`: **ไม่ต้อง bump** เพราะ schema ของ `buildings` ไม่เปลี่ยนและการเปลี่ยน save มี `RS3-0.2 → RS3-0.3` เป็น marker/migration อยู่แล้ว
...
3. **Phase (a) canonical socket**: executor ต้องปฏิเสธ socket ดิบที่ไม่ canonical เช่น side `E`/`S` (`'socket-shape'`) โดยไม่มี mutation; การทดสอบ pointer/snap และ snap-then-support ย้ายไป phase (c) Structural Snap ตาม §3 และ §10
4. **Retry-no-double-consume**: ส่ง `PLACE_STATION` ด้วย `placementId` เดิมซ้ำ 2 ครั้ง item ต้องลดครั้งเดียว มี station 1 ชิ้น ครั้งที่สองได้ `duplicate:true` (การ craft order หักไม้ตอน accept และการ placement ใช้เฉพาะ item ไม่หักไม้ซ้ำ) เพิ่มกรณีที่ log หลุดไปแล้ว (วางเกิน 64 ครั้ง) ต้องได้ `'duplicate-item'`; กรณี refund/คืนของย้ายไป P3 (removal/refund) และต้องระบุใหม่ว่าการ replay ที่มี command เดิมคืน `duplicate:true` หรือ `duplicate-item` ก่อน actor check
...
9. **RP1 disabled = baseline**: เมื่อใช้ seed เดียวกันและ RP1 off, hash ของ nodes, agents, stock (รวมข้อความ `ecologySignature`) ที่ tick 0 และ 720 ต้องเท่ากับ `main` `3044698`; RP1-off ต้องไม่มี house plan/placement แต่ full `serialize` ต่างได้เพราะ RS3 version และ field `placements` เปลี่ยน
10. **Save/load กลางการสร้างบ้าน**: ทำแบบ `tests/production-liveness.test.mjs` บรรทัด 67–77; เมื่อโหลดกลางการสร้าง ให้ derive แผนจาก world state ใหม่ ไม่มีชิ้นซ้ำ ไม้ไม่ถูกหักซ้ำ และ `serialize(continuous)===serialize(resumed)`
...
- `tests/ui-smoke.py` บรรทัด 92–101: phase (a) ต้องเช็คว่า panel Shelter และ shelter ghost หายไป (ไม่ใช่เช็คแค่ว่า BUILD ถูก reject); ไม่มี UI ทดแทน เพราะ RP1 builds houses automatically
...
9. ~~ควร bump `SAVE_VERSION` เป็น 0.6.0 แม้จะเลือกทางเลือก B ไหม~~ **ตัดสินแล้ว: ไม่ต้อง bump top-level `SAVE_VERSION`; ใช้ `RS3-0.2 → RS3-0.3` เป็น marker ของ save change ตามข้อ 7.2**
10. **Follow-up ไม่ block:** การเทียบ payload ของ `placementId` ที่ conflict ควรรวม `agentId` ด้วยหรือไม่ (ไม่เปลี่ยน acceptance ใน PR นี้)
