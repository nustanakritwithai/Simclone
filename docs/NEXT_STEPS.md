...
|---|---|---|
| UX legibility | **V0.6–V0.8 released; V0.9 merged awaiting public proof; V1.0 candidate:** Resource / Dropped Item / Event become direct world contexts on top of existing Clone/Structure interactions | Prove actual canvas taps resolve the intended object, event markers remain tappable without breaking Clone selection, resource/item panels read existing authoritative state only, pickup still uses PICKUP_ITEM, mobile has no overflow, and no second ledger or writer is introduced |
