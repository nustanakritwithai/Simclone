 ...
 */
import {createWorldMapView,shadowMovementCell,shadowRouteMovementCost} from './worldsim-map.mjs?v=0.5.0';
...
const nodeId=(x,y,width)=>y*width+x;
const point=(id,width)=>({x:id%width,y:Math.floor(id/width)});
...
export function shadowWeightedRoute(state,start,target){
  ...
  for(const p of [start,target])if(!Number.isInteger(p.x)||!Number.isInteger(p.y)||p.x<0||p.y<0||p.x>=bounds.w||p.y>=bounds.h)return null;
  const view=createWorldMapView(state),n=bounds.w*bounds.h,startId=nodeId(start.x,start.y,bounds.w),targetId=nodeId(target.x,target.y,bounds.w);
  ...
  while(true){
    ...
    const p=point(cur.id,bounds.w);
    for(const [dx,dy] of NEIGHBORS){
      const x=p.x+dx,y=p.y+dy;if(x<0||y<0||x>=bounds.w||y>=bounds.h)continue;
      const cell=shadowMovementCell(view,x,y);if(!cell||cell.movementCost===null)continue;
      const id=nodeId(x,y,bounds.w),step=Math.round(cell.movementCost*SCALE),cost=cur.cost+step;
      if(cost<dist[id]){dist[id]=cost;parent[id]=cur.id;heap.push({id,cost});}
    }
  }
  if(!Number.isFinite(dist[targetId]))return null;
  const path=[];for(let id=targetId;id!==startId;id=parent[id]){if(id<0)return null;path.push(point(id,bounds.w));}path.reverse();
