...
export const STATION_LIMITS=Object.freeze({maxStations:32,interactionRange:1});
...
const SUPPORT_KINDS=new Set(['WOOD_FOUNDATION','WOOD_WALL','WOOD_DOORWAY']);
...
const hasSupport=(s,x,y)=>(s.rustStations?.stations??[]).some(st=>SUPPORT_KINDS.has(st.kind)&&dist(st,{x,y})===1);
export function canPlaceStation(s,{agentId,itemInstanceId,x,y}={},isWalkable=()=>true){
  ...
  if(def.structurePiece&&!equippedHammer(s,agentId))return {ok:false,reason:'hammer'};
  if(def.placementRule==='ground'&&s.tiles?.[y*30+x]!=='grass')return {ok:false,reason:'foundation-ground'};
  if(def.placementRule==='supported'&&!hasSupport(s,x,y))return {ok:false,reason:'support'};
  ...
  return {ok:true,agentId,itemInstanceId,x,y,kind:def.stationProvided,buildingType:def.buildingType,structurePiece:def.structurePiece===true};
}
export function placeStationFromItem(s,data,isWalkable){
  ...
  const id=rs.nextStation++;rs.stations.push({id,kind:check.kind,buildingType:check.buildingType,x:check.x,y:check.y,complete:true,placedBy:check.agentId,placedTick:s.tick,structurePiece:check.structurePiece});
