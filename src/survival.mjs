...
import {kingdomStorageSnapshot} from './kingdom-storage.mjs?v=0.5.0';
...
export function survivalSummary(s){
  ...
  const kingdomMarket=kingdomMarketSnapshot({economy:kingdomEconomy,production:kingdomProduction});
  const kingdomStorage=kingdomStorageSnapshot({stock:s.stock,granary:false,warehouse:false});
  return {population:agents.length,hungry:agents.filter(a=>a.satiety<RULES.hungry).length,
    ...
    stock:{...s.stock},kingdomEconomy,kingdomProduction,kingdomLabor,kingdomMarket,kingdomStorage};
}
