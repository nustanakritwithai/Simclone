...
export function survivalSummary(s){
  ...
  return {population:agents.length,hungry:agents.filter(a=>a.satiety<RULES.hungry).length,
    ...
    food:s.stock.food,reservedMeals:book.meals.size,freeFood:Math.max(0,s.stock.food-book.meals.size),
    ...
    unfinished:s.buildings.filter(b=>!b.complete).length,stock:{...s.stock}};
}
