...
export const MENTORSHIP_VERSION='mentor-kf1-0.2';
...
export function createMentorLink(s,mentorId,studentId=null){
  ...
  const same=activeLinks(s).find(l=>l.mentorId===mentor.id&&l.studentId===student.id);
  if(same)return {ok:true,changed:false,linkId:same.id,message:'คู่นี้เป็น Mentor กันอยู่แล้ว'};
...
export function teachKnowledge(s,{mentorId,studentId,key}={}){
  ...
  const mentor=living(s,mentorId),student=living(s,studentId);
  ...
  if(link.taughtKeys.includes(key))
    return {ok:true,changed:false,reason:'already-taught',linkId:link.id,key,mentorId,studentId,message:'ความรู้นี้ถูกสอนใน Mentor link นี้แล้ว'};
  ...
  link.taughtKeys.push(key);
  while(link.taughtKeys.length>MENTORSHIP_RULES.taughtKeys)link.taughtKeys.shift();
