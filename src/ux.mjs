 ...
 function openSurvival(){
  ...
  api.openDialog('หมู่บ้านอยู่รอดอย่างไร','SURVIVAL CORE · '+VERSION,
   `<div class="life-summary"><div><small>อาหารที่ใช้ได้ตอนนี้</small><b>${v.freeFood} หน่วย</b></div><div><small>จองไว้ให้คนกิน</small><b>${v.reservedMeals} หน่วย</b></div></div>
    ...
    <div class="life-summary"><div><small>WorldSim WM4.2 · food ecology impact</small><b>shadow only</b></div><div><small>Ecology potential เฉลี่ย / median</small><b>${foodImpact.summary.averageEcologyPotential} / ${foodImpact.summary.medianEcologyPotential}</b></div></div>
    <div class="clone-skills"><div><span>Very low</span><b>${foodImpact.summary.bands['very-low']} nodes</b></div><div><span>Low</span><b>${foodImpact.summary.bands.low} nodes</b></div><div><span>Medium</span><b>${foodImpact.summary.bands.medium} nodes</b></div><div><span>High</span><b>${foodImpact.summary.bands.high} nodes</b></div><div><span>Potential min → max</span><b>${foodImpact.summary.minEcologyPotential} → ${foodImpact.summary.maxEcologyPotential}</b></div><div><span>Missing-weighted</span><b>${foodImpact.summary.missingWeightedEcologyPotential}</b></div><div><span>Legacy food regen</span><b>+3 / 120 ticks</b></div><div><span>Candidate unit formula</span><b>none</b></div></div>
    <p class="source-note">WM4.2 วัด distribution ของ ecology regeneration potential เทียบกับ food contract เดิมเท่านั้น · ยังไม่แปลงเป็นจำนวนหน่วย, ไม่เขียน node.amount และไม่เปลี่ยน regeneration cadence</p>
