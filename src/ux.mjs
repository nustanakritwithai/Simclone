...
export function installUX(api){
 ...
 function renderInspector(){
  ...
  if(tab==='skills')html=SKILLS.map(k=>{const p=a.skillProvenance?.bySkill?.[k],parts=[];
   ...
   if(chosen){html=`<div class="decision-callout">${icon('brain')}<div><small>เหตุผลจากการตัดสินใจล่าสุด</small><b>เลือก${LABELS[chosen.kind]} · ${chosen.score} คะแนน</b><p>เปรียบเทียบความต้องการ ความถนัด ทักษะ ความขาดแคลน อาชีพปัจจุบัน และระยะเดินจริง งานที่คนอื่นจองหรือทรัพยากรสำรองพอแล้วจะไม่ถูกเลือก</p></div></div>`;}
   ...
   if(chosen){const f=chosen.factors;html+=`<details class="score-details"><summary>ดูส่วนประกอบคะแนน</summary><dl>${[['พื้นฐาน',f.base],['ความต้องการ',f.need],['ความถนัด',f.goal],['ทักษะ',f.skill],['ความขาดแคลน',f.scarcity??0],['อาชีพเดิม',f.profession??0],['ความแปรผันแบบกำหนดได้',f.utilityJitter??0],['ระยะเดินจริง',f.distance]].map(([k,v])=>`<div><dt>${k}</dt><dd>${v>0?'+':''}${v}</dd></div>`).join('')}</dl></details><p class="source-note">ระยะเดินตอนเลือก ${chosen.travelSteps??'—'} ช่อง · ${a.task?'เลือกเมื่อ tick '+a.task.started:'งานล่าสุดสิ้นสุดแล้ว'} · คะแนนนี้มาจากกฎ CPU ไม่ใช่ข้อความคิดจาก LLM</p>`;}
