...
import {createFoodFormulaShadow} from './worldsim-food-regen-formula-shadow.mjs?v=0.5.0';
...
export function installUX(api){
 ...
 function openSurvival(){
  const s=api.read().state,v=survivalSummary(s),eco=createResourceEcologyShadow(s),pressure=shadowExistingResourcePressure(s,eco),hydro={summary:eco.hydrologySummary},regen=createResourceRegenerationShadow(s,eco),foodImpact=createFoodRegenerationImpact(s,regen),foodFormula=createFoodFormulaShadow(s,foodImpact);
  ...
  api.openDialog('หมู่บ้านอยู่รอดอย่างไร','SURVIVAL CORE · '+VERSION,
   `<div class="life-summary"><div><small>อาหารที่ใช้ได้ตอนนี้</small><b>${v.freeFood} หน่วย</b></div><div><small>จองไว้ให้คนกิน</small><b>${v.reservedMeals} หน่วย</b></div></div>
    ...
    <p class="source-note">WM4.2 วัด distribution ของ ecology regeneration potential เทียบกับ food behavior เดิมเท่านั้น · writer อยู่ที่ WorldSim WM4.1 แล้ว แต่ยังไม่เปลี่ยน node.amount หรือ cadence</p>
    <div class="life-summary"><div><small>WorldSim WM4.3 · candidate food formula</small><b>shadow only</b></div><div><small>Projected unit delta</small><b>${foodFormula.summary.unitDelta}</b></div></div>
    <div class="clone-skills"><div><span>Legacy boundary units</span><b>${foodFormula.summary.legacyBoundaryUnits}</b></div><div><span>Candidate boundary units</span><b>${foodFormula.summary.candidateBoundaryUnits}</b></div><div><span>Reduction ratio</span><b>${foodFormula.summary.reductionRatio}</b></div><div><span>Reduced nodes</span><b>${foodFormula.summary.reducedNodes}</b></div><div><span>Zero-regen candidate nodes</span><b>${foodFormula.summary.zeroNodes}</b></div><div><span>Formula</span><b>0 / 1 / 2 / 3 by ecology band</b></div></div>
    <p class="source-note">WM4.3 เป็น candidate เท่านั้น: very-low→0, low→1, medium→2, high→3 และ cap ตามพื้นที่ว่างของ node · WorldSim WM4.1 ยังใช้ +3/120 จริงเหมือนเดิมจนกว่า behavior gate จะผ่าน</p>
