 ...
 function openSurvival(){
  ...
  api.openDialog('หมู่บ้านอยู่รอดอย่างไร','SURVIVAL CORE · '+VERSION,
   `<div class="life-summary"><div><small>อาหารที่ใช้ได้ตอนนี้</small><b>${v.freeFood} หน่วย</b></div><div><small>จองไว้ให้คนกิน</small><b>${v.reservedMeals} หน่วย</b></div></div>
    ...
    <div class="life-summary"><div><small>Kingdom K5 · ราคากดดันสูงสุด</small><b>${escape(v.kingdomMarket.hottest?.good??'—')} ${v.kingdomMarket.hottest?`×${v.kingdomMarket.hottest.ratio}`:''}</b></div><div><small>ราคาเงา อาหาร / ไม้ / หิน</small><b>${v.kingdomMarket.prices.food} / ${v.kingdomMarket.prices.wood} / ${v.kingdomMarket.prices.stone}</b></div></div>
    <p class="source-note">K5 ใช้สูตรราคา Kingdom แบบ shadow: base × scarcity^0.75 (อาหารมี crowding modifier) ราคาเหล่านี้ยังไม่ซื้อขายอะไรจริง ไม่มีเงิน ภาษี treasury หรือ merchant จนกว่า market contract จะผ่าน verification</p>
    <div class="life-summary"><div><small>Kingdom K6 · ความจุคลัง</small><b>อาหาร ${v.kingdomStorage.caps.food} · วัตถุดิบ ${v.kingdomStorage.caps.wood}</b></div><div><small>อาหารเสื่อม/รอบ</small><b>${v.kingdomStorage.projectedFoodDecay} (${Math.round(v.kingdomStorage.decayRate*1000)/10}%)</b></div></div>
    <div class="clone-skills"><div><span>ใช้คลังอาหาร</span><b>${Math.round(v.kingdomStorage.utilization.food*100)}%</b></div><div><span>ใช้คลังไม้</span><b>${Math.round(v.kingdomStorage.utilization.wood*100)}%</b></div><div><span>ใช้คลังหิน</span><b>${Math.round(v.kingdomStorage.utilization.stone*100)}%</b></div><div><span>ข้อเสนออาคารคลัง</span><b>${v.kingdomStorage.needsGranary?'Granary ':''}${v.kingdomStorage.needsWarehouse?'Warehouse':''}${!v.kingdomStorage.needsGranary&&!v.kingdomStorage.needsWarehouse?'ยังไม่จำเป็น':''}</b></div></div>
    <p class="source-note">K6 ยังไม่หักอาหารเสื่อมหรือทิ้งของล้นจริง ค่าเหล่านี้เป็น projection ตาม Kingdom village storage: อาหาร cap 400, ไม้/หิน cap 200; Granary/Warehouse จะเพิ่ม cap ×1.8 และ Granary ลด decay จาก 1.2% → 0.4% เมื่อระบบอาคารเศรษฐกิจถูกเปิดจริงในเฟสหลัง</p>
