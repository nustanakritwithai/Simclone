...
const events={birth:'ชีวิตใหม่',skill:'พัฒนาทักษะ',knowledge:'ถ่ายทอดความรู้',mentor:'Mentor',build:'สิ่งปลูกสร้าง',career:'เปลี่ยนอาชีพ',death:'สูญเสีย',day:'วันใหม่'};
...
export function installUX(api){
 ...
 inspector.addEventListener('click',e=>{
  ...
  if(b.dataset.ux==='create-mentor'){
    const result=api.execute('CREATE_MENTOR_LINK',{mentorId:api.read().selected});
    api.toast(result.message);if(result.ok){api.save();renderInspector();}
  }
  if(b.dataset.ux==='end-mentor'){
    const result=api.execute('END_MENTOR_LINK',{linkId:Number(b.dataset.link)});
    api.toast(result.message);if(result.ok){api.save();renderInspector();}
  }
 });
 ...
 function renderInspector(){
  ...
  if(tab==='skills')html=SKILLS.map(k=>{const p=a.skillProvenance?.bySkill?.[k],parts=[];
   ...
   const links=s.mentorship?.links??[],mine=links.filter(l=>l.mentorId===a.id||l.studentId===a.id).slice().reverse();
   const activeStudent=links.find(l=>l.studentId===a.id&&l.endedTick===null);
   const rows=mine.map(l=>{
    const asMentor=l.mentorId===a.id,other=findPerson(s,asMentor?l.studentId:l.mentorId),active=l.endedTick===null;
    const role=asMentor?'Mentor ของ':'เรียนกับ';
    const taught=l.taughtKeys?.length?' · สอนแล้ว '+l.taughtKeys.length+' เรื่อง':'';
    const end=active&&a.alive?'<button class="secondary" data-ux="end-mentor" data-link="'+l.id+'">สิ้นสุด Mentor</button>':'';
    return '<div class="memory-item"><small>'+escape(active?'active':'สิ้นสุด tick '+l.endedTick)+'</small><b>'+escape(role+' '+(other?.name??('#'+(asMentor?l.studentId:l.mentorId))))+'</b><p class="source-note">เริ่ม tick '+l.createdTick+taught+(l.endReason?' · '+l.endReason:'')+'</p>'+end+'</div>';
   }).join('');
   const create=a.alive&&!activeStudent?'<button class="primary" data-ux="create-mentor">เป็น Mentor ให้คนใกล้สุด</button>':'';
   html=(rows||'<p class="empty-state">ยังไม่มีความสัมพันธ์ Mentor</p>')+create+
    '<p class="source-note">Mentor ถ่ายทอดเฉพาะ knowledge ที่ยืนยันแล้ว · ผู้เรียนรับเป็น UNVERIFIED และต้องตรวจเอง · การสอนไม่เพิ่ม Skill XP; XP ยังได้จากผลลัพธ์งานจริงเท่านั้น</p>';
  }
