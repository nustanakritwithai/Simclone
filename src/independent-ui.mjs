...
import {bestHouseholdTradeOpportunity} from './kingdom-household-trade.mjs?v=0.5.0';
...
export function installIndependentUI(api){
 ...
 function openHome(h){
  ...
  const topRecruitment=recruitment[0]??null,tradeOpportunity=owner?bestHouseholdTradeOpportunity(s,owner.id):null;
  const activeTrade=(s.householdTrade?.contracts??[]).find(c=>(c.originOwnerId===h.ownerId||c.destinationOwnerId===h.ownerId)&&c.status==='in-transit');
