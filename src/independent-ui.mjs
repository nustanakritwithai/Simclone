...
export function installIndependentUI(api){
 ...
 function update(){
  ...
  const lifePlan=life?.executablePlan?'<div data-executable-plan="'+esc(life.executablePlan.planId)+'"><span>เป้าหมาย <b>'+esc(life.executablePlan.goalLabel)+'</b> · '+esc(life.executablePlan.status)+'</span><span>ขั้นตอน <b>'+esc(life.executablePlan.step?.label??'ไม่มีขั้นตอน')+'</b> · '+esc(life.executablePlan.step?.status??'UNKNOWN')+'</span><small>Replan '+life.executablePlan.replanCount+'/'+life.executablePlan.maxReplans+'</small></div>':'';
  ...
  const lifeHtml=life?'<div class="autonomous-life-summary" data-autonomous-life="'+a.id+'"><small>Autonomous Life · VAL2</small><span>ตอนนี้ <b>'+esc(life.currentAction?.label??'ยังไม่มีงานปัจจุบัน')+'</b></span><span>แผนบ้าน <b>'+esc(life.homePlan.label)+'</b></span>'+lifePlan+lifePressure+lifeWhy+'</div>':'';
