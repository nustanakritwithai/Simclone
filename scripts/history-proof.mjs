/** 0.3.6+: long-run continuity plus direct current-rules archive equivalence. Not unlimited-time proof. */
...
import {allPeople,findPerson,retainedCount,HISTORY_LIMITS,compactRetired} from '../src/history.mjs';
...
function gameplayPerson(a){
 const copy=JSON.parse(JSON.stringify(a));
 delete copy.trace;
 delete copy.archived;
 return copy;
}
function gameplayProjection(s){
 const world=JSON.parse(serialize(s));
 delete world.agents;
 delete world.archive;
 return {world,people:allPeople(s).map(gameplayPerson)};
}
function proveArchiveEquivalence(s,seed,year){
 const hot=restore(serialize(s)),compacted=restore(serialize(s));
 assert.equal(hot.agents.length,HISTORY_LIMITS.hotRecords,'archive proof starts at hot-record boundary');
 const result=compactRetired(compacted);
 assert.ok(result.ok&&result.moved>0,'archive compaction must move retired records');
 assert.deepEqual(gameplayProjection(compacted),gameplayProjection(hot),'archive compaction preserves current-rules gameplay state');
 for(let offset=1;offset<=20;offset++){
  step(hot,360);step(compacted,360);
  assert.deepEqual(validate(hot),[],`hot archive-control state valid seed ${seed} +${offset}y`);
  assert.deepEqual(validate(compacted),[],`compacted archive-control state valid seed ${seed} +${offset}y`);
  assert.deepEqual(gameplayProjection(compacted),gameplayProjection(hot),`archive layout does not change gameplay seed ${seed} +${offset}y`);
 }
 return {startYear:year,hotRecords:HISTORY_LIMITS.hotRecords,moved:result.moved,horizonYears:20};
}
for(const seed of [230926,1,42,2026,90001]){
 ...
 let minPopulation=6,maxPopulation=6,maxHot=6,maxArchive=0,maxSampledSaveBytes=0,boundary=null,birthsAfter20=null,archiveProof=null;
 ...
 for(let year=1;year<=1800;year++){
  ...
  if(!archiveProof&&s.agents.length===HISTORY_LIMITS.hotRecords)archiveProof=proveArchiveEquivalence(s,seed,year);
 ...
 assert.ok(archiveProof,'direct current-rules archive equivalence proof must run');
 ...
 const row={seed,result:'SAT',years:1800,minPopulation,maxPopulation,extinct:false,manualClones:s.stats.cloned,starvationDeaths:0,ageDeaths:deaths.size,
  ...
  birthCount:people.filter(isAutonomousChild).length,boundary,birthsIn20YearsAfterBoundary:birthsAfter20,archiveEquivalence:archiveProof,saveContinuations:continuations,singleComparisons,
...
const report={engine:VERSION,result:'SAT',total:rows.length,passed:rows.filter(r=>r.result==='SAT').length,rows,
 scope:'Five fresh seeds, 1800 years each, no manual clone, resource injection, age reset, extra housing or resurrection. Each seed directly compares the same current-rules hot-record state with a compacted archive representation for 20 years, excluding only archive-only trace/marker storage fields. Invariants checked at year boundaries; save-size maxima sampled every 20 years. Runtime includes verification overhead. Native/physical-device performance is a separate claim.',
