...
function place(a,kind,socket){
  ...
  const r=command(s,'PLACE_STATION',{agentId:a.id,itemInstanceId:id,socket,placementId:'ic6b-browser:'+s.tick+':'+a.id+':'+id});
...
for(const row of [
  {fromId:subject.id,toId:owner.id,key:'fixture:from',delta:{trust:4,affinity:2}},
  {fromId:owner.id,toId:subject.id,key:'fixture:to',delta:{affinity:2}},
]){
  const r=recordRelationshipEvidence(s,{...row,kind:'fixture'});
  if(!r.ok)throw new Error('relationship failed '+JSON.stringify(r));
}
const joined=command(s,'JOIN_HOUSEHOLD',{agentId:subject.id,ownerId:owner.id});
