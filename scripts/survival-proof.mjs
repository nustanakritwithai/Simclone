...
import {createWorld,command,step,validate,living,capacity,survivalSummary,serialize,restore,ageYears} from '../src/engine.mjs';
...
for(const seed of [230926,1,42,2026,90001])for(const population of [6,12,36]){
 ...
 const alive=living(s),survivors=alive.length,initialSurvivors=alive.filter(a=>a.id<=population).length,foodAtEnd=s.stock.food,capacityAtEnd=capacity(s);
 const checkpoint=restore(serialize(s));step(s,60);step(checkpoint,60);assert.equal(serialize(s),serialize(checkpoint));
 const bounded=survivors<=Math.min(36,capacityAtEnd);
 const record={seed,population,elapsedDays:100,survivors,initialSurvivors,capacityAtEnd,lowestHP,maxHungry,maxNodeJobs,highestStock,foodAtEnd,foodTarget:survivalSummary(s).targets.food,elapsedMs:Math.round(performance.now()-start),
   result:initialSurvivors===population&&bounded?'SAT':'VIOL'};
...
for(const population of [6,12,36]){
 ...
 step(s,3600);const alive=living(s),survivors=alive.length,initialSurvivors=alive.filter(a=>a.id<=population).length,capacityAtEnd=capacity(s);
 results.push({fixture:'empty-food-crisis',population,elapsedDays:10,survivors,initialSurvivors,capacityAtEnd,foodAtEnd:s.stock.food,
   result:initialSurvivors===population&&survivors<=Math.min(36,capacityAtEnd)?'SAT':'VIOL'});
 console.log(JSON.stringify(results.at(-1)));
}
mkdirSync('evidence-survival',{recursive:true});
const report={scope:'V0.2 survival only; fixture-built starting population; proves initial-agent survival, bounded population/capacity, reservations and continuation while later autonomous births/housing may occur',total:results.length,passed:results.filter(x=>x.result==='SAT').length,results};
