...
test('IC1: two Clones can found distinct houses with distinct evidence-derived owners',()=>{
  ...
  const sa=personalHomeSite(s,a,walkable);
  assert.ok(sa);
  a.x=sa.origin.x;a.y=sa.origin.y;
  const ra=placeFoundation(s,a,sa);
  assert.equal(ra.ok,true);
  const sb=personalHomeSite(s,b,walkable);
  assert.ok(sb);
  assert.notDeepEqual(sa.origin,sb.origin);
  b.x=sb.origin.x;b.y=sb.origin.y;
  const rb=placeFoundation(s,b,sb);
