...
try:
 with sync_playwright() as p:
    ...
    for width,height in [(w,h) for w,h in [(1440,1000),(390,844),(320,740),(844,390)] if ARGS.width is None or ARGS.width==w]:
        ...
        plan_agent=page.evaluate('()=>simclone.snapshot().agents.find(a=>a.alive&&a.executablePlan)?.id??null')
        check(f'{width}: VAL2 earned world retains at least one executable plan',plan_agent is not None)
        if plan_agent!=1:
            select_person(page,plan_agent);page.wait_for_timeout(100)
            if width<=700 and not page.locator('#personal-home-summary').is_visible():page.locator('[data-ux="expand"]').click()
        plan=page.locator(f'[data-autonomous-life="{plan_agent}"] [data-executable-plan]')
        ...
        if plan_agent!=1:
            select_person(page,1);page.wait_for_timeout(100)
            if width<=700 and not page.locator('#personal-home-summary').is_visible():page.locator('[data-ux="expand"]').click()
