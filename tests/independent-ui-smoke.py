 ...
 with sync_playwright() as p:
    ...
    for width,height in [(w,h) for w,h in [(1440,1000),(390,844),(320,740),(844,390)] if ARGS.width is None or ARGS.width==w]:
        ...
        if width==1440:
            ...
            boot(page,social_fixture);pause(page);select_person(page,1);page.wait_for_timeout(150)
            page.wait_for_selector('.household-summary[data-household-owner="2"]')
            ...
            page.locator('[data-leave-household="1"]').click()
            page.wait_for_function('()=>!simclone.snapshot().social.residences.some(r=>r.agentId===1&&r.leftTick===null)')
            check('IC6B: LEAVE_HOUSEHOLD uses engine command and removes active residency',page.locator('.household-summary').count()==0)
            check('IC6B: relationship evidence remains after leaving',page.evaluate('simclone.snapshot().social.relations.some(r=>r.fromId===1&&r.toId===2&&r.trust===4&&r.affinity===2)'))
