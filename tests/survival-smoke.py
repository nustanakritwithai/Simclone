...
with sync_playwright() as p:
 ...
 page.set_content(HTML,wait_until='load');page.wait_for_function("window.simclone?.version==='0.5.0'")
 page.wait_for_selector('#boot-screen',state='detached');page.wait_for_timeout(500);page.locator('#pause').tap()
 check('old world migrates into lifecycle save schema',page.evaluate('simclone.snapshot().version')=='0.5.0')
 ...
 check('birth pacing text matches the four-year engine contract','เว้นการเกิดอย่างน้อย 4 ปีจำลอง' in page.locator('#dialog-body').inner_text() and 'เกิดได้สูงสุด 1 คนต่อปีจำลอง' not in page.locator('#dialog-body').inner_text())
 page.screenshot(path=str(OUT/'mobile-survival.png'))
 page.locator('#dialog-close').tap();page.locator('[data-nav="rust"]').tap()
 check('dedicated Rust dock opens the item catalog',page.locator('#dialog-title').inner_text()=='ไอเทมและการคราฟต์' and page.locator('[data-rust-catalog-item]').count()==5)
 page.locator('#dialog-close').tap();page.locator('[data-nav="people"]').tap();page.locator('[data-person="2"]').tap()
 page.locator('[data-nav="rust"]').tap()
 check('selected Clone exposes five bounded craft recipes',page.locator('[data-ux="craft-item"]').count()==5)
 stock_before=page.evaluate('simclone.snapshot().stock')
 page.locator('[data-ux="craft-item"][data-recipe="STONE_AXE"]').tap()
 rust=page.evaluate('simclone.snapshot()')
 check('Rust craft UI creates one authoritative order',len(rust['rustPossessions']['orders'])==1 and rust['rustPossessions']['orders'][0]['agentId']==2 and rust['rustPossessions']['orders'][0]['recipe']=='STONE_AXE')
 check('Rust craft UI commits materials exactly at acceptance',rust['stock']['wood']==stock_before['wood']-4 and rust['stock']['stone']==stock_before['stone']-2)
 page.locator('#dialog-close').tap();page.locator('[data-ux="why"]').tap()
