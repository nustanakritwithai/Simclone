...
test('entry ships boot fallback and valid stylesheet/module paths',()=>{const text=readFileSync(resolve(root,'index.html'),'utf8');assert.ok(text.includes('id="boot-screen"'));assert.ok(text.includes('src/boot.mjs?v=0.2.0'));for(const m of text.matchAll(/(?:src|href)="\.\/([^"?]+)(?:\?[^\"]+)?"/g))assert.ok(existsSync(resolve(root,m[1])),m[1]);});
