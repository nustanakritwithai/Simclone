...
import {VERSION,SAVE_VERSION,restore,serialize,step,validate} from '../src/engine.mjs';
const read=p=>readFileSync(new URL('../'+p,import.meta.url),'utf8');
const version=JSON.parse(read('package.json')).version;
test('release metadata and the entire runtime module graph agree',()=>{
 assert.equal(VERSION,version);assert.equal(version,'0.3.4');assert.equal(SAVE_VERSION,'0.2.0');
...
test('real V0.3.3 save is preserved byte-for-byte and continues deterministically',()=>{
 const text=read('tests/fixtures/legacy-0.3.3-save.json'),meta=JSON.parse(read('tests/fixtures/legacy-0.3.3-provenance.json'));
 ...
 const a=restore(text),b=restore(text);assert.equal(serialize(a),text);assert.equal(a.tick,720);
